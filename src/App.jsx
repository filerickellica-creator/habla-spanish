...
export default function App() {
  ...
  const clearApiKey = () => {
    localStorage.removeItem("habla_api_key");
    setApiKey("");
    setScreen("home");
  ...
  const callClaude = useCallback(async (userText, currentMessages, scen, lvl) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 1000, system: systemPrompt, messages: apiMessages }),
      });
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `You are a Spanish tutor helping a ${levelInfo.label} student practice conversation in the scenario: ${scenario?.label}.`,
          messages: [{ role: "user", content: `The AI just said: "${lastAI.text}"\n\nGive 2-3 short example responses in Spanish with English translations. Be encouraging and brief.` }],
        }),
      });
      const data = await res.json();
  ...
  }, [hintLoading, messages, level, scenario, apiKey]);
  ...
  const checkGrammar = useCallback(async (text, messageIndex) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-haiku-4-5-20251001", max_tokens: 1000,
          system: "You are a concise Spanish grammar tutor. Return a JSON object only — no extra text, no markdown. Fields: { \"hasErrors\": boolean, \"corrected\": string, \"errors\": [ { \"original\": string, \"fix\": string, \"explanation\": string } ], \"praise\": string }",
          messages: [{ role: "user", content: `Student said: "${text}"` }],
        }),
      });
      const data = await res.json();
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-haiku-4-5-20251001", max_tokens: 1000,
          system: systemPrompt,
          messages: [{ role: "user", content: "Start the conversation naturally with a greeting." }],
        }),
      });
      const data = await res.json();
